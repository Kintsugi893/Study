...
void toLower(std::string& word) {
    int lastRussianLetterIndex = 0; // Для поиска знаков в конце слова

    // Изменение регистра букв и поиск последней русской буквы
    for (int i = 0; i < word.length(); i++) {
        if (isRussianLetter(word[i])) {
            if (word[i] >= 'А' && word[i] <= 'Я') {
                word[i] = word[i] - 'А' + 'а'; // Приведение к нижнему регистру для заглавных букв
            }
            else if (word[i] == 'Ё') {
                word[i] = 'ё'; // Обработка буквы "Ё"
            }

            lastRussianLetterIndex = i;
        }
    }

    // Удаление знаков препинания в конце слова
    if (lastRussianLetterIndex != 0) {
        std::string newWord;
        for (int i = 0; i <= lastRussianLetterIndex; i++) {
            newWord += word[i];
        }
        word = newWord;  // Заменяем исходную строку новой строкой
    }
}
